...
<div align="center"> <h4> <b> Second simulation: $n_0 = 33$, $n_1 = 40$ and $k = 10$. </b> </h4></div>
...
Since we want to analyze how the ROIs of the two groups evolve in the time series, we actually are going to base our study on the auto-correlations.

What we are going to do is extract each ROIs' time series, compute it's auto-correlation and extract some statistic summaries from it.

For each time-series we decided to extract the following statistic summary:
...
Now we will try and use the Two-Sample-Testing procedure on our data, to check two different things:
